...
def fetch_botan():
    ...
    for link in soup.find_all("a"):
        if link.text.startswith("Botan") and not link.text.endswith(".asc"):
            ...
            match = re.match(
                r"Botan-(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)\.(?P<ext>.*)",
                link.text,
            )
            if match["major"] == "3":
                # TODO: Handle Botan-3
...
def fetch_openssl():
    ...
    tags += [
        tag_ref["ref"].split("/")[-1]
        for tag_ref in resp_tags.json()
        if tag_ref["ref"].startswith("refs/tags/openssl-")
        or tag_ref["ref"].startswith("refs/tags/OpenSSL_")
    ]
    tags = list(
        filter(
            lambda tag: "FIPS" not in tag
            and "reformat" not in tag
    ...
    for tag in tags:
        if tag.startswith("OpenSSL_"):
            match = re.match(
                r"OpenSSL_(?P<major>\d+)_(?P<minor>\d+)_(?P<patch>\d+)(?P<ext>.*)", tag
            )
...
def fetch_boringssl():
    ...
    with (
        ...
        sp.run(
            ["git", "clone", "https://boringssl.googlesource.com/boringssl", repodir]
        )
        ...
        for i, rev in enumerate(refs[upto_index:0:-40]):
            ...
            digest = (
                sp.check_output(["nix", "hash", "path", str(repodir)]).decode().strip()
            )
...
def fetch_libressl():
    ...
    for link in soup.find_all("a"):
        if link.text.startswith("libressl") and link.text.endswith(".tar.gz"):
            match = re.match(
                r"libressl-(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)\.tar.gz",
                link.text,
            )
            version = f"{match['major']}.{match['minor']}.{match['patch']}"
